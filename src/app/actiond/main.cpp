...
#include "action_host.h"          // Brings in ActionHost::runActions(), the actual load-and-run pipeline.
...
#include <QCommandLineOption>  // Provides QCommandLineOption, used to declare --pattern/--measurements/--actions.
#include <QCommandLineParser>  // Provides QCommandLineParser, used to parse and validate argv against those options.
#include <QFile>               // Provides QFile, used to read the actions JSON file from disk.
...
// Entry point: forces headless rendering, parses/validates arguments, then delegates to
// ActionHost for the actual load-and-run pipeline. Every Seamly2D exception is caught here so
// none can escape main() and terminate the process without the required JSON error on stderr.
int main(int argc, char *argv[])
{
    ...
    // Command-line contract: --pattern, --measurements, and --actions are all required, with no
    // positional arguments and no shorthand flags, per the task's specified interface.
    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral( // Shown by --help; documents the exact contract this binary expects.
        "Runs a JSON actions script against a Seamly2D pattern file, headlessly, and prints the result as JSON."));
    ...
    if (!parser.isSet(patternOption) || !parser.isSet(measurementsOption) || !parser.isSet(actionsOption))
    {
        QTextStream(stderr) << QStringLiteral( // A plain usage line, not JSON: this is an argument-syntax error, not a runtime failure.
            "Usage: actiond --pattern <path.val> --measurements <path.smis|.smms|.vst> --actions <path.json>") << Qt::endl;
