...
QJsonDocument ActionEngine::run(const QJsonDocument &script, const ActionContext &ctx, bool abortOnFirstError,
                                 const AfterActionFn &afterAction)
{
    ...
    for (const QJsonValue &actionValue : actions) // Walk every entry in the "actions" array, in script order.
    {
        ...
        if (!handler) // An empty std::function means no handler is registered under this name.
        {
            ...
            if (afterAction) // Fires for every dispatched action, even one that failed to resolve to a handler -- keeps the "once per action, always" contract simple for any future caller.
            {
                afterAction(op, false);
            }
        ...
        if (afterAction) // See AfterActionFn's own comment (action_engine.h) for why this must fire here -- immediately, before the loop moves on -- rather than once after the whole script finishes.
        {
            afterAction(op, result.ok);
        }
